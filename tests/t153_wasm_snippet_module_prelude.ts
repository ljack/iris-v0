...
export const t153_wasm_snippet_module_prelude = {
  ...
  fn: async () => {
    ...
    const expected = [
      ...
      '(type $temp_reset_type (func (result i64)))',
      ...
      '(import "host" "str_concat_temp" (func $host.str_concat_temp (type $str_concat_type)))',
      '(import "host" "str_eq" (func $host.str_eq (type $str_eq_type)))',
      '(import "host" "temp_reset" (func $host.temp_reset (type $temp_reset_type)))',
      ...
      '  (local $mem_pages i64)',
      '  (local $mem_bytes i64)',
      '  (local $end_with_temp i64)',
      '  (local $delta i64)',
      '  (local $grow_pages i64)',
      ...
      '  (local.set $end_with_temp (i64.add (local.get $end) (i64.const 65536)))',
      '  (local.set $mem_pages (i64.extend_i32_u (memory.size)))',
      '  (local.set $mem_bytes (i64.mul (local.get $mem_pages) (i64.const 65536)))',
      '  (if (i64.gt_u (local.get $end_with_temp) (local.get $mem_bytes)) (then (local.set $delta (i64.sub (local.get $end_with_temp) (local.get $mem_bytes))) (local.set $grow_pages (i64.div_u (i64.add (local.get $delta) (i64.const 65535)) (i64.const 65536))) (drop (memory.grow (i32.wrap_i64 (local.get $grow_pages))))))',
      '  (i32.store (i32.const 0) (i32.wrap_i64 (local.get $end)))',
      '  (local.get $ptr)',
      ')',
      '',
      ')',
