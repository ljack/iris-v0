...
tests.forEach((t, i) => {
    ...
    const fileName = `t${num}.ts`;
    const varName = `t${num}`;
    const filePath = path.join(testsDir, fileName);

    const fileContent = `import { TestCase } from '../src/test-types';

export const ${varName}: TestCase = {
  name: '${t.name.replace(/'/g, "\\'")}',
  expect: '${t.expect.replace(/'/g, "\\'").replace(/\n/g, "\\n")}',
  source: \`${t.source.replace(/`/g, "\\`")}\`${t.fs ? `,
  fs: ${JSON.stringify(t.fs)}` : ''}
};
`;

    fs.writeFileSync(filePath, fileContent);
    console.log(`Created ${fileName}`);
    exportsList.push(`export { ${varName} } from './${varName.replace('.ts', '')}';`);
});
